...
int knapsack(int profits[maxi], int weights[maxi], int target, int n){
    int i, j;
    int k[n+1][target+1];
    for(i=0; i<=n; i++){
        for(j=0; j<=target; j++){
            if(i==0 || j==0){
                ...
                k[i][j] = max(k[i-1][j], profits[i-1]+k[i][j-weights[i-1]]);// same as knapsack but just k[i][j-weights[i-1]]
            }
            else{
                k[i][j] = k[i-1][j];
            }
